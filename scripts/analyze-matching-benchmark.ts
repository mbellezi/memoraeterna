...
const noteRelations=results.noteRelations.map((r:any)=>{const a=notes.get(r.source_atomic_note_id),b=notes.get(r.target_atomic_note_id);const x=root(a.created_from_source_item_id),y=root(b.created_from_source_item_id);return{...r,sourceKey:keys.get(a.created_from_source_item_id),targetKey:keys.get(b.created_from_source_item_id),sourceIdea:a.idea_statement,targetIdea:b.idea_statement,roots:pair(x,y),inPartition:selectedPair(x,y),classification:classify(pair(x,y))};}).filter((r:any)=>r.inPartition);
...
const sourceSweep=[0.75,0.8,0.825,0.85,0.875,0.9].map(minimumImportance=>({minimumImportance,...measure(sourceRelations.filter((r:any)=>r.importance>=minimumImportance&&r.confidence>=settings.sourceRelationSettings.minConfidence))}));
...
const summary={phase,partition,capturedAt:results.capturedAt,sourceRecords:results.sources.length,rootCount:results.sources.filter((s:any)=>!s.parent_source_item_id).length,notes:results.notes.length,entities:results.entities.length,
  ...
  usage,noteThresholdSweep:noteSweep,noteWeightSweep:weightSweep,sourceImportanceSweep:sourceSweep,
