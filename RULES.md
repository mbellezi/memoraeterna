...
Se houver conflito entre este arquivo e uma etapa do plano, pare e registre a divergencia antes de implementar.
...
- Vetores: `pgvector`.
- Grafo: Apache AGE, inicialmente apenas como camada simples de projecao/consulta.
- Contratos: Zod.
...
- Extensao Chrome e plugin Obsidian nunca acessam PGlite diretamente.
...
- Cada `.md` gerenciado deve ter frontmatter minimo com:
  ...
  - `memora_sync_version`;
  - `memora_content_hash`.
- O plugin Obsidian pode monitorar arquivos e frontmatter, mas nao pode acessar PGlite diretamente.
...
- Credenciais nunca ficam em texto puro no PGlite.
...
- Deve rodar apenas no main process ou em workers controlados pelo main process.
...
- Nao deve acessar PGlite diretamente.
