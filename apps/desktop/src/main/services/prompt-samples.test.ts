...
function model(){const calls:Array<{id:string,input:string}>=[];return {calls,describePromptRoute:vi.fn(async()=>({provider:'openai-codex',modelId:'qwen3-embedding-fixture',repository:'fixture'})),runDefaultTask:vi.fn(async(task:string,input:string)=>{
 ...
 else if(id==='organization.evolution.support')output={supported:true,checkedTargets:['new_topic','new_index'],issues:[]};
 else if(['organization.temporal','organization.procedure','organization.consolidation'].includes(id)){
  const policy=JSON.parse(input.split('POLICY: ')[1]!.split('\nGROUP:')[0]!);const interpretation={statement:'In2024 I preferred short study sessions.',perspective:'user_statement',context:'historical',eventTime:{precision:'year',value:'2024'},validFrom:null,validUntil:null,publicationTime:null,relationships:[],procedure:id==='organization.procedure'?{steps:['Try three short sessions'],workedWhen:'The supplied author felt less tired',assumptions:['Self report'],limits:['Retention was not measured'],corrections:[]}:null,originalHandles:['e1','e2']};
  output={explanation:'Read original experience with qualifications',topics:[{handle:'new_topic',title:'Experience',purpose:'Read original experience',sections:[{id:null,title:'Experience',markdown:'In2024 I preferred short sessions. [e1]',originalHandles:['e1','e2'],...(id==='organization.consolidation'?{}:{interpretation})}],links:['new_index']}],indexes:[{handle:'new_index',title:'Originals',purpose:'Read exact originals',owner:{topic:'new_topic'},groups:[{id:null,title:'Sources',explanation:null,originalHandles:[],targets:id==='organization.consolidation'?['r1','r2','r3','r4']:['r1','r2']}]}],...(id==='organization.consolidation'?{noteEvolution:{version:'note-evolution-v1',kind:'cross_source',noteIds:policy.selectedNotes.map((n:any)=>n.id),reason:'Preserve distinct attributed originals',notes:[{title:'Experience',ideaStatement:'Two supplied observations',bodyMarkdown:'Short sessions were preferred; retention was not measured.',originalHandles:['e1','e2']}]}}:{})};
 }
