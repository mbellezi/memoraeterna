...
export async function buildObsidianMirror(pool: PgPool, root: string, scope: ObsidianWikiScope, config: ObsidianLayoutConfig, options: { migrate?: boolean; excluded?: Set<string>; assets?: MirrorAsset[] } = {}): Promise<MirrorTarget[]> {
  const repo=createObsidianWikiRepository(pool), registry=await createObsidianSyncRepository(pool).list(), sources=await repo.sources(scope.sourceIds,scope.includeDescendants), notes=await repo.notes(sources.map(s=>s.id));
  ...
  for (const id of await repo.pages(scope.pageIds)) {
    ...
    if (page.evidence.every(e=>allowed.has(e.sourceItemId)) && (!page.archived || registry.some(f=>f.memoraId===id))) pages.push(page);
  }
  ...
  for(const note of notes){
    ...
    if(originals.some(e=>!allowed.has(e.sourceId)))throw new Error('obsidianWiki.errors.binding');
    ...
    const originalReferences=originals.map(e=>`${link(e.sourceId,e.sourceTitle)} · ${e.documentId} · ${e.id} · ${e.sourceSpanId??''} · ${safeWikiLabel(e.locator??'')} · ${t(e.current?'obsidianWiki.current':'obsidianWiki.historical')}\n\n${quote(e.content)}`).join('\n\n');
    add({id:note.id,type:'atomic_note',revision:note.updatedAt.toISOString(),title:note.title,sourceId:note.sourceId,...(evidence?{documentId:evidence.documentId}:{}),editorial:`# ${markdownHeading(note.title)}\n\n${note.bodyMarkdown}\n\n`,generated:[noteState,source?link(source.id,source.title):'',originalReferences||t('obsidianWiki.unavailable'),list(memberships)].filter(Boolean).join('\n\n')});
  }
  for(const page of pages){
    ...
    add({id:page.id,type:'wiki_page',revision:page.revisionId,title:page.title,linkMap,editorial:`# ${markdownHeading(page.title)}\n\n`+renderedSections.map(s=>`${sectionStart(s.id)}\n## ${markdownHeading(s.title)}\n\n${references(s.markdown)}\n${sectionEnd(s.id)}\n\n^memora-section-${s.id}\n`).join('\n')+'\n',generated:[page.archived?t('maintenance.archivedRecoverable'):t(('wiki.'+page.review) as MessageKey),navigation,list(pages.filter(p=>!p.archived&&(p.parentId===page.id||p.automatic?.role.kind==='collection'&&p.automatic.role.owner?.id===page.id))),...(page.automatic?.links??[]).map(typed),sectionReferences,citations,link(INDEX,t('wiki.title'),'wiki')].filter(Boolean).join('\n\n')});
  }
