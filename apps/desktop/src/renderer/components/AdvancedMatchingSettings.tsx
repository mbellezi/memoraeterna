...
import { AtomicNoteMatchingSettingsSchema, recommendedMatchingConfiguration } from "@app/domain";
...
export function AdvancedMatchingSettings({ settings, onChange, t }: {
  ...
  const { atomicNoteMatchingSettings: defaultAtomicNoteMatchingSettings, canonicalMatchingSettings: defaultCanonicalMatchingSettings, sourceRelationSettings: defaultSourceRelationSettings } = recommendedMatchingConfiguration;
  const resetLabel = t("settings.matching.resetDefault");
    ...
    <div className="mt-4 grid min-w-0 gap-5">
      ...
      <fieldset className="grid min-w-0 gap-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          ...
          ] as const).map(([key,min,max]) => <NumericSetting key={key} label={label(key)} value={notes[key]} defaultValue={defaultAtomicNoteMatchingSettings[key]} resetLabel={resetLabel} min={min} max={max}
        ...
        {(["requireReranking", "includeWeakTypes"] as const).map((key) => <label key={key} className="flex items-center justify-between gap-3 text-sm">
          {label(key)}<span className="flex items-center gap-2"><MatchingResetButton label={`${label(key)}: ${resetLabel}`} onReset={() => updateNotes({ [key]: defaultAtomicNoteMatchingSettings[key] })} /><Switch checked={notes[key]} onChange={(event) => updateNotes({ [key]: event.target.checked })} /></span>
        </label>)}
      ...
      <fieldset className="grid min-w-0 gap-3 border-t border-slate-200 pt-4 dark:border-slate-800">
        ...
        {(["withEmbeddingAndGraph", "withEmbedding", "withGraph", "textAndMetadata"] as const).map((group) => <div key={group} className="grid gap-2">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {Object.entries(notes[group]).map(([key, value]) => <NumericSetting key={key} label={label(key)} value={value} defaultValue={(defaultAtomicNoteMatchingSettings[group] as Record<string,number>)[key]!} resetLabel={resetLabel} min={0} max={1} step={0.01}
              onCommit={(next) => updateNotes({ [group]: { ...notes[group], [key]: next } })} />)}
          </div>
        </div>)}
        <NumericSetting label={label("rerankerWeight")} value={notes.rerankerWeight} defaultValue={defaultAtomicNoteMatchingSettings.rerankerWeight} resetLabel={resetLabel} min={0} max={1} step={0.01} onCommit={(value) => updateNotes({ rerankerWeight: value })} />
      </fieldset>
      <fieldset className="grid min-w-0 gap-3 border-t border-slate-200 pt-4 dark:border-slate-800">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {([["relationTypeCandidateLimit",1,20], ["entityCandidateLimit",1,20], ["confirmationBatchSize",1,12], ["confirmationMaxCharacters",2000,24000]] as const).map(([key,min,max]) =>
            <NumericSetting key={key} label={label(key)} min={min} max={max} value={settings.canonicalMatchingSettings[key]} defaultValue={defaultCanonicalMatchingSettings[key]} resetLabel={resetLabel}
      ...
      <fieldset className="grid min-w-0 gap-3 border-t border-slate-200 pt-4 dark:border-slate-800">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {([["reciprocalRankConstant",1,200], ["evidenceChunksPerSource",1,12], ["evidenceMaxCharacters",300,6000], ["summaryMaxCharacters",300,6000], ["noteRelationsPerPair",0,30], ["maxOutputTokens",512,16384]] as const).map(([key,min,max]) =>
            <NumericSetting key={key} label={label(key)} min={min} max={max} value={settings.sourceRelationSettings[key]} defaultValue={defaultSourceRelationSettings[key]} resetLabel={resetLabel}
...
function NumericSetting({ label, value, defaultValue, resetLabel, min, max, step = 1, onCommit }: {
  label: string; value: number; defaultValue: number; resetLabel: string; min: number; max: number; step?: number; onCommit: (value: number) => boolean | void;
}) {
  ...
  return <label className="grid min-w-0 gap-1 text-xs"><span className="flex items-center justify-between gap-2">{label}<MatchingResetButton label={`${label}: ${resetLabel}`} onReset={() => { setDraft(String(defaultValue)); onCommit(defaultValue); }} /></span>
