...
import { Archive, FileX, HardDrive, Save, ServerCog, Trash2 } from "lucide-react";
import type { MessageKey } from "@app/i18n";
import type { StorageSettings } from "../../shared/ipc";
    ...
    <form className="grid gap-6" onSubmit={submit}>
      <section className="grid gap-4 border-b border-slate-200 pb-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-slate-950">{t("integrations.obsidianPlugin")}</h2>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <label className="flex min-h-12 items-center gap-3 rounded-md border border-slate-200 px-3">
            <Switch
              checked={settings.obsidianSyncEnabled}
              onChange={(event) => update("obsidianSyncEnabled", event.target.checked)}
            />
            <span className="text-sm font-medium text-slate-800">{t("settings.storage.obsidianSyncEnabled")}</span>
          </label>
          <label className="flex min-h-12 items-center gap-3 rounded-md border border-slate-200 px-3">
            <Switch
            ...
            />
            <span className="text-sm font-medium text-slate-800">{t("settings.storage.obsidianSyncPaused")}</span>
          </label>
          <div className="grid gap-2">
                ...
                <button
                  ...
                  className={
                    settings.deletionPolicy === policy
                      ? "flex h-9 items-center justify-center bg-cyan-700 text-white"
                      : "flex h-9 items-center justify-center bg-white text-slate-700 hover:bg-slate-100"
                  }
      ...
      <section className="grid gap-4 border-b border-slate-200 pb-6">
        <div className="flex items-center gap-2">
          <HardDrive className="h-5 w-5 text-emerald-700" aria-hidden="true" />
          <h2 className="text-lg font-semibold text-slate-950">{t("settings.storage.title")}</h2>
        </div>
        <label className="flex min-h-12 items-center gap-3 rounded-md border border-slate-200 px-3">
          <Switch
          ...
          />
          <span className="text-sm font-medium text-slate-800">{t("settings.storage.copyUploadedFilesEnabled")}</span>
        </label>
        <div className="grid gap-2">
