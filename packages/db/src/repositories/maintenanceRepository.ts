...
const scheduleRow=(r:Data)=>({id:r.id,revision:r.revision,policy:r.policy,nextAt:stamp(r.next_at),lastRunId:r.last_run_id,lastError:r.last_error,updatedAt:stamp(r.updated_at)});
export function createMaintenanceRepository(pool:PgPool){
  ...
  const cancel=async(db:PgClient,id:string)=>{
    const row=(await db.query(`update maintenance_runs set status='canceled',updated_at=now() where id=$1 and status not in ('applied','rejected','sample_passed') returning job_id,checkpoint`,[id])).rows[0];
    if(row&&!row.checkpoint.callPending&&!row.checkpoint.calls){const released={calls:0,tokens:0,spend:0,inspected:row.checkpoint.inspected??0,changes:0};await db.query('update maintenance_runs set reservation=$2 where id=$1',[id,released]);await db.query('update maintenance_budget_reservations set reservation=$2 where run_id=$1',[id,released]);}
  ...
  return {
    async schedules(){return (await pool.query("select * from maintenance_schedules where policy->>'version' is distinct from 'automatic-maintenance-v1' and retired_at is null order by updated_at desc limit 100")).rows.map(scheduleRow);},
    async save(id:string|undefined,expected:number|undefined,policy:Data,nextAt:string){return tx(async db=>{
      ...
      if(id){const existing=(await db.query('select * from maintenance_schedules where id=$1 for update',[id])).rows[0];if(!existing||existing.policy.version==='automatic-maintenance-v1'||existing.revision!==expected)throw new Error('maintenance.errors.conflict');
        for(const run of (await db.query(`select id from maintenance_runs where schedule_ids ? $1 and status not in('applied','rejected','sample_passed','no_change','canceled')`,[id])).rows)await cancel(db,run.id);
        await db.query('update maintenance_schedules set revision=revision+1,policy=$2,next_at=$3,last_error=null,updated_at=now() where id=$1',[id,policy,nextAt]);
    ...
    async canceledJobs(scheduleId:string){return (await pool.query("select r.job_id from maintenance_runs r join jobs j on j.id=r.job_id where r.schedule_ids ? $1 and r.status='canceled' and j.status='running'",[scheduleId])).rows.map(r=>r.job_id as string);},
    async scheduleError(ids:string[],error:string|null){await pool.query('update maintenance_schedules set last_error=$2 where id=any($1::uuid[])',[ids,error]);},
    async scope(scope:Data){
      ...
      return {wholeLibrary:scope.wholeLibrary,includePageDescendants:false,pageIds:scope.wholeLibrary?[]:[...pages].filter(id=>!excluded.includes(id)).sort(),sourceIds:scope.wholeLibrary?[]:[...sources].sort(),excludedPageIds:excluded.sort()};
    },
    ...
    async admit(snapshot:Data,checkpoint:unknown,occurrences:Array<{scheduleId:string;revision:number;key:string;from:string;until:string;nextAt:string}>,manual:boolean){return tx(async db=>{
      await lock(db);await db.query("select pg_advisory_xact_lock(hashtextextended('organization-settings',0))");
      const activeConfiguration=(await db.query("select coalesce((select value->>'revisionId' from settings where key='organization.active'),(select revision_id::text from organization_settings_activations order by created_at desc,id desc limit 1)) as id")).rows[0]?.id??null;if(activeConfiguration!==snapshot.configurationId)throw new Error('maintenance.errors.conflict');
      ...
      if(active&&(active.snapshot.configurationHash!==snapshot.configurationHash||hash(active.snapshot.instructions)!==hash(snapshot.instructions)||hash({...active.snapshot.policy,name:null,enabled:null,cadence:null})!==hash({...snapshot.policy,name:null,enabled:null,cadence:null})))throw new Error('maintenance.errors.wait');
      let id=active?.id as string|undefined;
      if(!id){
        const amount=snapshot.policy.modelEnabled?snapshot.policy.budget:{...snapshot.policy.budget,calls:0,tokens:0,spend:0,changes:0};
        ...
        for(const key of ['calls','tokens','spend','inspected','changes']){const limit=snapshot.policy.periodBudget[key];if(amount[key]!==0&&limit!==null&&(amount[key]===null||reserved.some(r=>r.reservation[key]===null)||reserved.reduce((n,r)=>n+(r.reservation[key]??0),0)+amount[key]>limit))throw new Error('maintenance.errors.period');}
        ...
        await db.query('insert into maintenance_runs(id,job_id,scope_key,period,schedule_ids,snapshot,checkpoint,reservation) values($1,$2,$3,$4,$5,$6,$7,$8)',[id,job,snapshot.scopeKey,snapshot.period,JSON.stringify(occurrences.map(o=>o.scheduleId)),snapshot,checkpoint,snapshot.policy.modelEnabled?snapshot.policy.budget:{...snapshot.policy.budget,calls:0,tokens:0,spend:0,changes:0}]);
      ...
      for(const o of occurrences){await db.query('insert into maintenance_occurrences(schedule_id,occurrence_key,due_from,due_until,run_id) values($1,$2,$3,$4,$5)',[o.scheduleId,o.key,o.from,o.until,id]);
        await db.query('update maintenance_schedules set last_run_id=$2,last_error=null,next_at=case when $4 then next_at else $3 end where id=$1',[o.scheduleId,id,o.nextAt,manual]);}
      if(active)await db.query('update maintenance_runs set schedule_ids=$2 where id=$1',[id,JSON.stringify([...new Set([...active.schedule_ids,...occurrences.map(o=>o.scheduleId)])])]);
      return id;
    });},
    ...
    async reservePeriod(id:string,period:string){return tx(async db=>{await lock(db);const r=(await db.query('select * from maintenance_runs where id=$1 for update',[id])).rows[0];if(!r||r.status==='canceled')throw new Error('maintenance.errors.revoked');if(r.snapshot.sample)return;
      ...
      for(const k of ['calls','tokens','spend','inspected','changes']){const cap=r.snapshot.policy.periodBudget[k],amount=r.reservation[k];if(amount!==0&&cap!==null&&(amount===null||rows.some(row=>row.reservation[k]===null)||rows.reduce((n,row)=>n+(row.reservation[k]??0),0)+amount>cap))throw new Error('maintenance.errors.period');}
    ...
    async decisions(keys:string[],pageIds:string[],cutoff:Date){return (await pool.query(`select key,page_id as "pageId",outcome from maintenance_decisions where key=any($1::text[]) or page_id=any($2::uuid[]) and created_at>$3 and outcome in('awaiting_review','applied','rejected')`,[keys,pageIds,cutoff])).rows;},
    async settle(id:string,proposal:unknown,checkpoint:Data,decisions:Array<{key:string;pageId:string}>,sample:boolean){return tx(async db=>{await lock(db);const r=(await db.query('select * from maintenance_runs where id=$1 for update',[id])).rows[0];if(!r||r.status==='canceled')throw new Error('maintenance.errors.revoked');
      const reservation={calls:checkpoint.calls,tokens:checkpoint.calls?checkpoint.inputTokens!==null&&checkpoint.outputTokens!==null?checkpoint.inputTokens+checkpoint.outputTokens:r.reservation.tokens:0,spend:checkpoint.calls?checkpoint.cost:0,inspected:checkpoint.inspected,changes:(proposal as Data).operations.length};
      if(!sample){await db.query('update maintenance_runs set reservation=$2 where id=$1',[id,reservation]);await db.query('update maintenance_budget_reservations set reservation=$2 where run_id=$1',[id,reservation]);}
      const status=sample?'sample_passed':(proposal as Data).operations.length?'awaiting_review':'no_change';await db.query('update maintenance_runs set proposal=$2,checkpoint=$3,status=$4,updated_at=now() where id=$1',[id,proposal,checkpoint,status]);if(!sample)for(const d of decisions)await db.query('insert into maintenance_decisions(key,page_id,run_id,outcome) values($1,$2,$3,$4) on conflict(key) do update set outcome=excluded.outcome',[d.key,d.pageId,id,(proposal as Data).operations.some((op:Data)=>op.pageId===d.pageId)?status:'no_change']);});},
    ...
    async inspect(scope:Data,kind:string,cursor:string|null,cutoff:string,limit:number){
      const pageScope=`($1::boolean or p.id=any($2::uuid[]) or exists(select 1 from wiki_evidence e where e.page_id=p.id and e.source_item_id=any($3::uuid[]) and exists(select 1 from jsonb_array_elements(r.content->'sections') sec where sec->'evidenceIds' ? e.id::text)) and not exists(select 1 from wiki_evidence outside where outside.page_id=p.id and not outside.source_item_id=any($3::uuid[]) and exists(select 1 from jsonb_array_elements(r.content->'sections') sec where sec->'evidenceIds' ? outside.id::text))) and not p.id=any($4::uuid[]) and not p.archived and p.created_at<=$5`;
      ...
      if(kind==='page'){
        ...
        const total=Number((await pool.query(`select count(*) n from wiki_pages p join wiki_page_revisions r on r.id=p.current_revision_id where ${pageScope} and ($6::uuid is null or p.id>$6)`,args.slice(0,6))).rows[0].n);return {rows,total};
      }
      ...
      if(kind==='source'){
        ...
        const total=Number((await pool.query(`select count(*) n from source_items s where ${where} and ($6::uuid is null or s.id>$6)`,args.slice(0,6))).rows[0].n);return {rows,total};
      }
      ...
      const total=Number((await pool.query(`select count(*) n from atomic_notes n join source_items s on s.id=n.created_from_source_item_id where ${where} and ($6::uuid is null or n.id>$6)`,args.slice(0,6))).rows[0].n);return {rows,total};
    },
    ...
    async review(id:string,decision:string,validate:(run:Data)=>Array<{id:string;expectedRevisionId:string;content:any;evidenceChunkIds:string[]}>){return tx(async db=>{
      ...
      if(decision==='reject'){await db.query("update maintenance_runs set status='rejected',updated_at=now() where id=$1",[id]);await db.query("update maintenance_decisions set outcome='rejected',created_at=now() where run_id=$1 and outcome='awaiting_review'",[id]);return;}
      ...
      for(const op of run.proposal.operations)if(op.type==='archive'&&(await db.query("select id from wiki_pages where not archived and (parent_id=$1 or exists(select 1 from wiki_page_revisions r where r.id=wiki_pages.current_revision_id and r.content->'collectionIds' ? $1::text)) limit 1",[op.pageId])).rows.length)throw new Error('maintenance.errors.protected');
      ...
      await db.query("update maintenance_runs set status='applied',updated_at=now() where id=$1",[id]);await db.query("update maintenance_decisions set outcome='applied',created_at=now() where run_id=$1 and outcome='awaiting_review'",[id]);
    });}
  };
}
