...
export function createOrganizationRepository(pool:PgPool) {
  ...
  return {
    ...
    async list(){await pool.query("update organization_runs r set status='applied',updated_at=now() where status<>'applied' and exists(select 1 from organization_receipts where run_id=r.id)");return (await pool.query(`select r.id,r.status,case when r.checkpoint->>'callPending'='true' then case when r.checkpoint->>'waitingForModel'='true' then 'waiting' else 'active' end else 'none' end as "modelState",r.snapshot->'baseContent'->>'title' as title,(r.snapshot->>'sample')::boolean as sample,receipt.revision_id as "receiptRevisionId",r.created_at as "createdAt",r.updated_at as "updatedAt" from organization_runs r left join organization_receipts receipt on receipt.run_id=r.id where r.snapshot->>'version'<>'wiki-curator-v2' order by r.created_at desc limit 50`)).rows.map(r=>({...r,createdAt:stamp(r.createdAt),updatedAt:stamp(r.updatedAt)}));},
