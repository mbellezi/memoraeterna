...
export function createOrganizationRepository(pool:PgPool) {
  ...
  return {
    ...
    async create(snapshot:unknown,checkpoint:unknown){
      return transaction(async db=>{
        const curator=snapshot as {version?:string;inputFingerprint?:string;policy?:{id:string}};
        if(curator.version==='wiki-curator-v2'){await db.query("select pg_advisory_xact_lock(hashtextextended($1,0))",['wiki-admission:'+curator.policy!.id+':'+curator.inputFingerprint]);const prior=(await db.query("select id from organization_runs where snapshot->>'version'='wiki-curator-v2' and snapshot->>'inputFingerprint'=$1 and snapshot->'policy'->>'id'=$2 and status<>'applied' order by created_at desc limit 1",[curator.inputFingerprint,curator.policy!.id])).rows[0];if(prior)return prior.id as string;}
    ...
    async curatorIds(){return (await pool.query("select id from organization_runs where snapshot->>'version'='wiki-curator-v2' order by created_at desc limit 30")).rows.map(r=>r.id as string);},
    async list(){await pool.query("update organization_runs r set status='applied',updated_at=now() where status<>'applied' and exists(select 1 from organization_receipts where run_id=r.id)");return (await pool.query(`select r.id,r.status,case when r.checkpoint->>'callPending'='true' then case when r.checkpoint->>'waitingForModel'='true' then 'waiting' else 'active' end else 'none' end as "modelState",r.snapshot->'baseContent'->>'title' as title,(r.snapshot->>'sample')::boolean as sample,receipt.revision_id as "receiptRevisionId",r.created_at as "createdAt",r.updated_at as "updatedAt" from organization_runs r left join organization_receipts receipt on receipt.run_id=r.id where r.snapshot->>'version' not in('wiki-curator-v2','wiki-bootstrap-v1') order by r.created_at desc limit 50`)).rows.map(r=>({...r,createdAt:stamp(r.createdAt),updatedAt:stamp(r.updatedAt)}));},
    ...
    async hasProviderAudit(id:string){return !!(await pool.query('select 1 from organization_steps where run_id=$1 and ai_task_run_id is not null limit 1',[id])).rows.length;},
    ...
    async persistCuratorCandidate(id:string,proposal:unknown,checkpoint:unknown){return transaction(async db=>{const run=(await db.query('select status from organization_runs where id=$1 for update',[id])).rows[0];if(!run||!['queued','analyzing'].includes(run.status))throw new Error('organization.errors.canceled');await db.query('insert into organization_proposals(run_id,proposal) values($1,$2) on conflict(run_id) do update set proposal=excluded.proposal where organization_proposals.decision is null',[id,proposal]);await db.query('update organization_runs set checkpoint=$2,updated_at=now() where id=$1',[id,checkpoint]);});},
    ...
    async retry(id:string){return transaction(async db=>{
      ...
      if(!run||!['failed','canceled'].includes(run.status)||(run.status==='failed'&&run.checkpoint.error&&!['organization.errors.failed','organization.errors.uncertain',...(run.snapshot.version==='wiki-curator-v2'?['organization.errors.policy','organization.errors.canceled']:[])].includes(run.checkpoint.error))||run.checkpoint.calls>=run.snapshot.limits.modelCalls)throw new Error('organization.errors.budget');
