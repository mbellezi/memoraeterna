...
export const promptDefinitions: PromptDefinition[] = shippedPromptBodies.map(row => {
    ...
    return PromptDefinitionSchema.parse({ version: 'prompt-catalog-v1', id: row.id, category: [family, ...parts.slice(1, -1)], title, purpose: promptMetadataLabels('purposes', family), caller: row.caller, task, supportsDomain: ['organization', 'consultation', 'maintenance'].includes(family), defaultVersion: row.id==='organization.evolution.support'?'4':['organization.temporal','organization.procedure','organization.consolidation','organization.evolution.repair','organization.evolution.support'].includes(row.id)?'3':['organization.curator','consultation.knowledge','consultation.investigation','consultation.gaps','organization.temporal','organization.procedure','organization.consolidation','organization.evolution.repair'].includes(row.id)?'2':'1', fields: [{ id: 'body', template: row.template, editable: true, variableKeys: [...new Set(row.keys)], requiredContractIds: row.keys.includes('output_contract' as never) ? ['output_contract'] : [] }, ...('contract' in row ? [{ id: 'output_contract', template: row.contract, editable: false, variableKeys: [], requiredContractIds: [] }] : [])], variables: [...new Set(row.keys)].map(key => { const v = variable(key, row.id), format = 'formats' in row ? (row.formats as Record<string, {
