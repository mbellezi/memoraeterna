import { z } from 'zod';
import { AutomaticMaintenancePreviewSchema } from './automatic-maintenance.js';
...
export const CuratorStartSchema = z.object({ policyId:z.string().uuid(), policyRevisionId:z.string().uuid(), sourceIds:z.array(z.string().uuid()).min(1).max(100), noteIds:z.array(z.string().uuid()).max(100).default([]),evidenceChunkIds:z.array(z.string().uuid()).min(1).max(12).optional() }).strict();
...
export const CuratorSnapshotSchema=z.object({version:z.literal(curatorVersion),optionalReferenceCoverage:z.object({includedEntities:z.number().int(),omittedEntities:z.number().int()}).default({includedEntities:0,omittedEntities:0}),requiredPageIds:z.array(z.string().uuid()).max(3).default([]),maintenanceRunId:z.string().uuid().nullable().default(null),parentBootstrapId:z.string().uuid().nullable().default(null),causalRunId:z.string().uuid().nullable().default(null),causalGroupId:z.string().uuid().nullable().default(null),policy:AutomaticWikiPolicySchema,sourceIds:z.array(z.string().uuid()),profile:OrganizationProfileSchema,promptPin:PromptPinSchema,contentLanguage:z.enum(['en','pt-BR','it','fr','es']),evidence:z.array(OrganizationEvidenceSchema).max(12),references:z.array(CuratorTargetSchema).max(300),pages:z.array(WikiPageSchema).max(100),inputFingerprint:z.string(),groupId:z.string().uuid(),outputTokens:z.number().int().positive().max(8192),limits:AutomaticWikiPolicySchema.shape.limits,admittedAt:z.string(),participation:OrganizationParticipationSchema.nullable().default(null),sample:z.literal(false).default(false)}).strict();
...
export const CuratorCheckpointSchema=z.object({executionBaseMs:z.number().nonnegative().nullable().default(null),executionAdmittedAt:z.string().nullable().default(null),version:z.literal(curatorVersion),calls:z.number().int().nonnegative(),tools:z.number().int().nonnegative(),repairs:z.number().int().min(0).max(1),startedAt:z.string(),callPending:z.boolean(),waitingForModel:z.boolean(),proposal:CuratorChangeSetSchema.nullable(),supportAssessment:CuratorSupportAssessmentSchema.nullable().default(null),previousOutput:z.string().max(90000),validationIssues:z.array(z.object({path:z.string().max(300),message:z.string().max(500)}).strict()).max(12).default([]),error:z.string().nullable(),reportedInputTokens:z.number().nonnegative(),reportedOutputTokens:z.number().nonnegative(),costEstimate:z.number().nonnegative(),usageCounts:z.object({input:z.number().int(),output:z.number().int(),cost:z.number().int()}),usageIncomplete:z.boolean()}).strict();
...
export const CuratorPolicyDraftSchema=z.object({timezone:z.string().optional(),policyId:z.string().uuid().optional(),wholeLibrary:z.boolean().default(false),sourceIds:z.array(z.string().uuid()).max(100),includeDescendants:z.boolean().default(false),excludedSourceIds:z.array(z.string().uuid()).max(100).default([]),profileOverrideId:z.string().uuid().nullable().default(null),limits:AutomaticWikiPolicySchema.shape.limits.default(initialCuratorLimits)}).strict();
export const CuratorPreviewSchema=z.object({maintenance:AutomaticMaintenancePreviewSchema.nullable().default(null),policy:AutomaticWikiPolicySchema,sourceTitles:z.array(z.object({id:z.string().uuid(),title:z.string()})),profile:OrganizationProfileSchema.nullable(),promptIds:z.array(z.string()),previewHash:z.string(),modelError:z.string().nullable()}).strict();
export const CuratorCommandSchema=z.discriminatedUnion('command',[
 ...
 z.object({command:z.literal('resume'),policyId:z.string().uuid()}).strict(),
...
export const WikiAssessmentInputSchema=z.object({policyId:z.string().uuid(),after:z.string().uuid().nullable().default(null),limit:z.number().int().min(1).max(100).default(50)}).strict();
export const WikiCoverageStateSchema=z.enum(['cataloged','awaiting','analyzing','partial','integrated','attention','stale']);
export const WikiAssessmentSourceSchema=z.object({id:z.string().uuid(),title:z.string(),originals:z.number().int(),notes:z.number().int(),summaries:z.number().int(),vectors:z.number().int(),relations:z.number().int(),pages:z.number().int(),fingerprint:z.string(),coverage:WikiCoverageStateSchema,covered:z.number().int(),protectedPages:z.number().int()}).strict();
export const WikiAssessmentSchema=z.object({scopeConflicts:z.array(z.object({pageId:z.string().uuid(),title:z.string(),excludedSources:z.array(z.object({id:z.string().uuid(),title:z.string()}).strict())}).strict()).max(12).default([]),policyId:z.string().uuid(),policyRevisionId:z.string().uuid(),sources:z.array(WikiAssessmentSourceSchema),next:z.string().uuid().nullable(),totals:z.object({sources:z.number().int(),originals:z.number().int(),cataloged:z.number().int(),integrated:z.number().int(),attention:z.number().int()}).strict(),groupOriginalLimit:z.number().int(),readOnly:z.literal(true)}).strict();
export const WikiBootstrapCheckpointSchema=z.object({version:z.literal('wiki-bootstrap-v1'),lastTickAt:z.string().nullable().default(null),phase:z.enum(['originals','notes']).default('originals'),noteCursor:z.number().int().default(0),state:z.enum(['running','paused','complete','attention','canceled']),afterSource:z.string().uuid().nullable(),sourceId:z.string().uuid().nullable(),sourceFingerprint:z.string().nullable(),afterChunk:z.string().uuid().nullable(),nextChunk:z.string().uuid().nullable(),childRunId:z.string().uuid().nullable(),pendingOriginals:z.number().int().default(0),sourceCovered:z.number().int(),completedSources:z.number().int(),catalogedSources:z.number().int(),completedGroups:z.number().int(),attentionSources:z.number().int(),error:z.string().nullable()}).strict();
export const WikiBootstrapSchema=z.object({id:z.string().uuid(),status:z.string(),snapshot:z.object({version:z.literal('wiki-bootstrap-v1'),participation:OrganizationParticipationSchema.nullable().default(null),noteIds:z.array(z.string().uuid()).max(1000).default([]),policy:AutomaticWikiPolicySchema,promptPin:PromptPinSchema,profile:OrganizationProfileSchema,contentLanguage:z.string(),sourceIds:z.array(z.string().uuid()),admittedAt:z.string()}).strict(),checkpoint:WikiBootstrapCheckpointSchema,createdAt:z.string(),updatedAt:z.string()}).strict();
export const WikiBootstrapStartSchema=z.object({noteIds:z.array(z.string().uuid()).max(1000).default([]),policyId:z.string().uuid(),policyRevisionId:z.string().uuid(),sourceIds:z.array(z.string().uuid()).max(1000).default([])}).strict();
export const WikiCollectionCommandSchema=z.discriminatedUnion('command',[
 z.object({command:z.literal('assessment'),input:WikiAssessmentInputSchema}).strict(),
 z.object({command:z.literal('bootstrap'),input:WikiBootstrapStartSchema}).strict(),
 z.object({command:z.literal('bootstraps'),policyId:z.string().uuid().optional(),after:z.string().uuid().optional()}).strict(),
 ...(['bootstrapGet','bootstrapPause','bootstrapResume','bootstrapCancel']as const).map(command=>z.object({command:z.literal(command),id:z.string().uuid()}).strict()),
 z.object({command:z.literal('drainImpacts')}).strict()
]);
export const WikiCollectionResponseSchema=z.union([WikiAssessmentSchema,WikiBootstrapSchema,z.array(WikiBootstrapSchema),z.null()]);
export type WikiBootstrap=z.infer<typeof WikiBootstrapSchema>;
